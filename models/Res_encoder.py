...
class Res_encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(1, self.in_channel, kernel_size=7, stride=1,
                               padding=0, bias=False)
        self.bn1 = nn.InstanceNorm2d(self.in_channel)
        self.relu = nn.ReLU(inplace=True)
        # self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 128, blocks_num[0], stride=2)
...
def encoder_make():
    return Res_encoder(BasicBlock, [2, 2, 2, 2])
